...
const PotentialVendorsState = (props) => {
    ...
    useEffect(() => {
        ...
        if (isSubscribed) {
            fetch('http://192.168.0.182:54321/api/get-potential-vendors-gen', {
                method: 'POST',
                headers: {
...
const PotentialVendorVersion = (props) => {
    ...
    useEffect(() => {
        ...
        if (isExpanded) {
            ...
            fetch('http://192.168.0.182:54321/api/get-price-offer-content', {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Content-Type': 'application/json',
...
export const VendorInfo = (props) => {
    ...
    useEffect(() => {
        ...
        fetch('http://192.168.0.182:54321/api/get-potential-vendor-info', {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json',
            ...
            <div className="pontential-vendor-files-container">
                {
                    vendorData.files.map(file =>
                        <div key={file.id}>
                            <a rel="noopener noreferrer" target="_blank" href={`http://192.168.0.182:54321/original/${file.name}`}>
                                {
                                    file.ext === 'pdf'
                                    ? <AiOutlineFilePdf size="40" title={file.name}/>
                                    : <img src={`http://192.168.0.182:54321/thumbs/${file.name}`} alt={`${file.name}`} />
                                }
                            </a>
                        </div>
                    )
